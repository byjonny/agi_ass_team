...
const IDEA_BANK = [
    {
        title: 'Inbox triage squad',
        agent: 'One agent summarizes unread threads, one drafts replies, one flags anything emotional or time-sensitive.',
        firstStep: 'Create three agents and route all incoming messages through the team before responding.',
    },
    {
        title: 'Bug bash desk',
        agent: 'A reviewer agent scans recent changes while a fixer agent handles the smallest actionable issue.',
        firstStep: 'Point the team at one repo folder and ask for a 30-minute cleanup pass.',
    },
    {
        title: 'Morning operator',
        agent: 'A daily agent checks schedules, stale tasks, and unanswered messages, then sends one concise brief.',
        firstStep: 'Add a schedule that runs every weekday morning with a "what needs attention?" prompt.',
    },
    {
        title: 'Rubber duck channel',
        agent: 'A lightweight agent asks one clarifying question before proposing a plan, keeping big work from drifting.',
        firstStep: 'Connect it to a chat channel and make it the default first responder.',
    },
    {
        title: 'Release captain',
        agent: 'One agent writes release notes, another checks docs, another looks for risky config or secret leaks.',
        firstStep: 'Run the team before tagging a release and require all three to produce a short sign-off.',
    },
    {
        title: 'Tiny office DJ',
        agent: 'A morale agent renames boring tasks, writes tiny status blurbs, and keeps the team board less gray.',
        firstStep: 'Give it write access only to task descriptions, never production code.',
    },
];

function printIdea(args) {
    const showAll = args.includes('--all') || args.includes('-a');
    const ideas = showAll
        ? IDEA_BANK
        : [IDEA_BANK[Math.floor(Math.random() * IDEA_BANK.length)]];

    for (const [index, idea] of ideas.entries()) {
        if (showAll) log(BLUE, `Idea ${index + 1}/${ideas.length}`);
        log(GREEN, idea.title);
        console.log(`  Agent setup: ${idea.agent}`);
        console.log(`  First step:  ${idea.firstStep}`);
        if (index < ideas.length - 1) console.log('');
    }
}
...
switch (command) {
    ...
    case 'idea': case 'ideas': case 'spark':
        printIdea(restArgs);
        break;
    ...
    case '--help': case '-h': case 'help':
        ...
        console.log('  idea [--all]             Print a quick agent/team idea');
